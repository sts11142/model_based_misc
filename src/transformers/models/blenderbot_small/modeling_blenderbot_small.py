...
class BlenderbotSmallEncoderLayer(nn.Module):
    def __init__(self, config: BlenderbotSmallConfig):
        ...
        self.embedding_proj = nn.Linear(self.embed_dim, config.d_model)
...
class BlenderbotSmallDecoderLayer(nn.Module):
    def __init__(self, config: BlenderbotSmallConfig):
        ...
        self.encoder_attn_empathy = BlenderbotSmallAttention(
            self.embed_dim,
            config.decoder_attention_heads,
            dropout=config.attention_dropout,
            is_decoder=True,
    ...
    def forward(
        ...
        cog_ref_ctx: Optional[torch.Tensor] = None,
        past_key_value: Optional[Tuple[torch.Tensor]] = None,
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if getattr(self.config, "gradient_checkpointing", False):
                ...
                layer_outputs = decoder_layer(
                    ...
                    comet_mask_st=comet_mask_st,
                    strategy_embs = strategy_embs,  # この時点でh^gになっている
                    cog_ref_ctx=mixed_hidden_states,
...
class MLP(nn.Module):
    def __init__(self, config: BlenderbotSmallConfig):
        ...
        input_num = 5
        input_dim = input_num * config.d_model
        hid_num = 3
        hid_dim = hid_num * config.d_model
...
class BlenderbotSmallForConditionalGeneration(BlenderbotSmallPreTrainedModel):
    ...
    def __init__(self, config: BlenderbotSmallConfig):
        ...
        # Encoderのstartegy部分からの流用
        self.strategy_head = nn.Linear(config.d_model, 8) # 512次元 -> 8次元
        self.batchNorm_strategy = nn.LayerNorm(8)
        self.strategy_embedding = nn.Embedding(8 + 1, config.d_model, 8)
    ...
    def forward(
        ...
        # EEモジュールの出力をSLモジュールの入力に繋ぐ
        if comet_embs is not None and comet_embs_st is not None:
            strategy_logits = self.strategy_head(cog_ref_ctx[:, 0, :]) # paperのC_1に対応する
        else:
            strategy_logits = None
        ...
        decoder_outputs = self.model.decoder(
            input_ids=decoder_input_ids,
            attention_mask=decoder_attention_mask,
            encoder_hidden_states=cog_ref_ctx,
